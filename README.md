...
- **Battery** — fully wired:
  ...
  - A dropdown with a big charge icon, % on-AC/on-battery state, power details
...
- The disk bar's two lines are threshold-tinted on the **value only**: the `U:`
  ...
  higher-is-better (calm at/above `mildLimit`, mild from `calmLimit`, alarming
  below it). Both reuse the `calmLimit`/`mildLimit` config (default 30/60).
